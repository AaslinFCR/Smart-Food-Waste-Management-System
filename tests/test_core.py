...
def test_leftover_dishes_and_feedback_are_available() -> None:
    engine = FoodWasteEngine(Path("data"))
    dashboard = engine.snapshot()
    assert dashboard["dishes"]
    dish = dashboard["dishes"][0]["name"]
    updated = engine.record_feedback(dish, "like")
    assert updated["feedback"]["likes"] >= 1
